...
Call it by hand to finish a stacked branch, or from a longer run that
builds a stack one unit at a time.
...
4. **Exercise what this branch touched**, and nothing wider.

   This repository has no test suite and no CI, so there is no command to
   run here yet. Import or run the code the branch changes, enough to show
   it is not broken, and say in the report exactly what you did. If a test
   suite is added later, run the targeted tests for the changed files and
   name them in the report.

   If something fails, stop and report it. Fixing it is a code change nobody
   has reviewed, and by this point the unit is being landed rather than
   built. A failure *here* means the work was finished broken, which a human
   should see.
...
5. **Push the whole stack and open or update its pull requests.**
   ...
   as drafts, which is the right default on a stack: every later rebase
   rewrites the branches above this one, so a pull request is not worth a
   reviewer's attention until the branch below it has settled.
...
9. **Write the title and the body.** The title takes the stack key form
   ...
   **British English throughout**: organisation, behaviour, recognise,
   licence as the noun. Identifiers, route paths and library names keep whatever
   ...
   **One idea per bullet, and no sentence that restates another.** Do not
   ...
   - **`## LLM decisions`** — the choices you made that a human might have
     made differently. This is where a reviewer's judgement is the
   ...
   - **`## Risks`** — security, leaked credentials, posting to a live
     account by mistake, anything that could go wrong beyond the code being
     incorrect. **Always present**,
...
10. **Mark it ready only if `ready` was given**, and only if step 6 reported
   ...
   If it is already out of draft, leave it and say so. This is the last
   step; do not merge it.
